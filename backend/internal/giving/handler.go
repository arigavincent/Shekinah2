...
func buildGivingDocumentPDF(title string, subtitle string, transaction gin.H) ([]byte, error) {
	lines := []string{
		...
		"Status: " + documentStatusLabel(stringValue(transaction["status"])),
		"Category: " + stringValue(transaction["category"]),
		"Amount: " + formatKesDocument(transaction["amount"], stringValue(transaction["currency"])),
	...
	lines = append(lines, "", "Generated by the Shekinah Sons Global giving system.")
